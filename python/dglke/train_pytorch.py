...
try:
    from yext_common.utils import LoggingUtils

    print("---------- 00000 ----------")
    mlog = LoggingUtils.MetricLogger("dgl-ke")
    mlog.enable(["wandb"])
except ModuleNotFoundError:
    mlog = logging.Logger("stdout")
    # 0 is NOTSET: https://docs.python.org/3/library/logging.html#logging-levels
    mlog.setLevel(level=0)
    mlog.log = mlog.warning  # hack to match apis
...
def train(
    ...
    for step in range(0, args.max_step):
        ...
        if (step + 1) % args.log_interval == 0:
            if (client is not None) and (client.get_machine_id() != 0):
                ...
                for k in logs[0].keys():
                    ...
                    if not isinstance(mlog, LoggingUtils.MetricLogger):
                        mlog.log(
                            "[proc {}][Train]({}/{}) average {}: {}".format(
                                rank, (step + 1), args.max_step, k, v
                            )
                        )
                    else:
                        # print(rank, (step + 1), args.max_step, k, v)
                        metrics = {
                            "process number ": rank,
                            "step": step + 1,
                            # "metric name": k,
                ...
                if not isinstance(mlog, LoggingUtils.MetricLogger):
                    mlog.log(
                        "[proc {}][Train] {} steps take {:.3f} seconds".format(
                            rank, args.log_interval, time.time() - start
                        )
                    )
                    mlog.log(
                        "[proc {}]sample: {:.3f}, forward: {:.3f}, backward: {:.3f}, update: {:.3f}".format(
                    ...
                    metrics = {
                        "process number ": rank,
                        "interval length": args.log_interval,
        ...
        if (
            ...
            print(
                "[proc {}]validation take {:.3f} seconds:".format(
    ...
    if args.dataset == "wikikg90M":
        with th.no_grad():
            ...
            if mode == "Valid":
                ...
                input_dict = {}
                input_dict["h,r->t"] = {
                    "t_correct_index": th.cat(answers, 0),
                    "t_pred_top10": th.cat(logs, 0),
                }
                th.save(
                    input_dict, os.path.join(args.save_path, "test_{}.pkl".format(rank))
                )
    else:
        with th.no_grad():
            logs = []
...
def train_mp(
    ...
    train(
        args, model, train_sampler, valid_samplers, rank, rel_parts, cross_rels, barrier
    )
...
def dist_train_test(
    ...
    train(
        args, model, train_sampler, None, rank, rel_parts, cross_rels, barrier, client
    )
    ...
    if (client.get_machine_id() == 0) and (
        client.get_id() % args.num_client == 0
    ):  # pull full model from kvstore
        # Pull model from kvstore
        args.num_test_proc = args.num_client
        dataset_full = dataset = get_dataset(
